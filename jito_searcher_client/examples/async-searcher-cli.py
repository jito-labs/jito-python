...
def mempool_accounts(client: SearcherServiceStub, accounts: List[str]):
    ...
    res: UnaryStreamCall = client.SubscribePendingTransactions(PendingTxSubscriptionRequest(accounts=accounts))
    event_loop.run_until_complete(res.read())
...
def tip_accounts(client: SearcherServiceStub):
    ...
    """
    accounts = event_loop.run_until_complete(client.GetNextScheduledLeader(NextScheduledLeaderRequest()))
    print(f"{accounts=}")
